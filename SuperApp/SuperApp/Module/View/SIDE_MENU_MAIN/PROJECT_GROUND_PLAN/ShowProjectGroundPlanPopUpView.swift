...
struct ShowProjectGroundPlanPopUpView: View {
    ...
    @Binding var dataFloor: [floorModel]

    @State var temp_selectedType = ""
    @State var temp_selectedFloor = ""
    @State var temp_selectedTypeID = 0
    @State var temp_selectedFloorID = 0
        ...
        ZStack {
            if isShow {
                Text("").onAppear {
                    ...
                    self.temp_selectedFloor = self.selectedFloor
                    self.temp_selectedTypeID = self.selectedTypeID
                    self.temp_selectedFloorID = self.selectedFloorID
                }
            }
            VStack {
                Spacer()
                VStack(spacing: 0) {
                    ...
                    ZStack {
                        ...
                        HStack {
                            Spacer()
                            Button(action: {
                                self.selectedFloor = self.temp_selectedFloor
                                self.selectedType = self.temp_selectedType
                                self.selectedFloorID = self.temp_selectedFloorID
                    ...
                    ScrollView {
                        VStack (spacing: 0) {
                            ForEach(dataFloor, id: \.id) { data in
                                PopUpCustomView(
                                    selectedFloorID: self.$temp_selectedFloorID,
                                    selectedFloor: self.$temp_selectedFloor,
                                    data: data
                                )
                            }
                        }
...
struct PopUpCustomView: View {
    @Binding var selectedFloorID: Int
    @Binding var selectedFloor: String
    @State var data: floorModel

    var body: some View {
        VStack (spacing: 0) {
            Button(action: {
                self.selectedFloorID = self.data.id
                self.selectedFloor = self.data.title
            }) {
                HStack {
                    ...
                    ZStack {
                        ...
                        .opacity(selectedFloorID == data.id ? 1 : 0)
                }.background(Color.white)
            }.buttonStyle(ButtonTapScale())
                .animation(.easeInOut(duration: 0.1))
                .background(Color.white)
