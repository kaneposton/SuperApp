...
struct VideoLibraryView: View {
    ...
    @State var listVideo = [List_video]()
        ...
        ZStack {
            if isHasData {
                if listVideo.count != 0 {
                    ForEach(listVideo, id: \.self) { data in
                        VideoScrollLibrary(
            ...
            self.videoViewModel.isHasData.sink { rs in
                self.listVideo = self.videoViewModel.LibraryData?.list_video ?? []
                for i in 0 ..< self.listVideo.count {
                    self.listVideo[i].index = i
                }
                self.isHasData = true
                ...
                DragGesture()
                    ...
                    .onEnded({ value in
                        ...
                        if self.viewStatePicture.width < (-self.spaceImage * self.pictureIndex) {
                            if self.pictureIndex < CGFloat(integerLiteral: self.listVideo.count - 1) {
...
struct VideoScrollLibrary: View {
    @State var label = ""
    @State var urlVideo = ""
    @Binding var pictureIndex: CGFloat
    var data: List_video = List_video()
    @State var index = -1
    var body: some View {
        ZStack {
            VStack (spacing: 0) {
...
struct PlayerUIView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: UIViewControllerRepresentableContext<PlayerUIView>) -> PlayerUIView.UIViewControllerType {
        let controller = AVPlayerViewController()
        var url = URL(string: "https:")!
        do {
            url = URL(string: urlVideo)!
        } catch {
    ...
    func updateUIViewController(_ uiViewController: PlayerUIView.UIViewControllerType, context: UIViewControllerRepresentableContext<PlayerUIView>) {
